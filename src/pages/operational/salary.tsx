              ...
              <Dialog.Content>
                <Dialog.Header>
                  <Dialog.Title className="font-semibold">
                    {form.id === "-" ? "Create New Salary" : "Edit Salary"}
                  </Dialog.Title>
...
export default AuthPage(SalaryPage, ["CASHIER", "OWNER"]);
...
const useSalary = () => {
  ...
  const create = async () => {
    try {
      const res = await api.post("/operational/salary", form);
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/operational/salary/${form.id}`, form);
      await fetching();
      makeToast("success", res?.data?.message);
    } catch (error) {
      throw error;
  ...
  const confirmDelete = async (item: Transaction) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/operational/salary/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
