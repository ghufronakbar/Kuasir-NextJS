              ...
              <Dialog.Content>
                <Dialog.Header>
                  <Dialog.Title className="font-semibold">
                    {form.id === "-" ? "Create New Rnd" : "Edit Rnd"}
                  </Dialog.Title>
...
export default AuthPage(RndPage, ["CASHIER", "OWNER"]);
...
const useRnd = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await api.get<Api<Capital[]>>("/capital/rnd");
  ...
  const create = async () => {
    try {
      const res = await api.post("/capital/rnd", form);
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/capital/rnd/${form.id}`, form);
      await fetching();
      makeToast("success", res?.data?.message);
    } catch (error) {
      throw error;
  ...
  const confirmDelete = async (item: Capital) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/capital/rnd/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
