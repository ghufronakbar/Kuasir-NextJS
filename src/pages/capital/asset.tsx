...
const THEAD = ["No", "Asset", "Note", "Amount", "Created At", ""];
              ...
              <Dialog.Content>
                <Dialog.Header>
                  <Dialog.Title className="font-semibold">
                    {form.id === "-" ? "Create New Asset" : "Edit Asset"}
                  </Dialog.Title>
                ...
                <Dialog.Body>
                  ...
                  >
                    <Label className="mt-2 font-medium">Asset</Label>
                    <input
...
export default AuthPage(AssetPage, ["CASHIER", "OWNER"]);
...
const useAsset = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await api.get<Api<Capital[]>>("/capital/asset");
    setData(response.data.data);
    setLoading(false);
  };

  const onClickDetail = (item: Capital) => {
    setOpen(true);
    setForm({
      amount: item.amount,
  ...
  const create = async () => {
    try {
      const res = await api.post("/capital/asset", form);
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/capital/asset/${form.id}`, form);
      await fetching();
      makeToast("success", res?.data?.message);
    } catch (error) {
      throw error;
