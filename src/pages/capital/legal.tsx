...
const THEAD = ["No", "Legal", "Note", "Amount", "Created At", ""];
              ...
              <Dialog.Content>
                <Dialog.Header>
                  <Dialog.Title className="font-semibold">
                    {form.id === "-" ? "Create New Legal" : "Edit Legal"}
                  </Dialog.Title>
                ...
                <Dialog.Body>
                  ...
                  >
                    <Label className="mt-2 font-medium">Legal</Label>
                    <input
...
export default AuthPage(LegalPage, ["CASHIER", "OWNER"]);
...
const useLegal = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await api.get<Api<Capital[]>>("/capital/legal");
  ...
  const create = async () => {
    try {
      const res = await api.post("/capital/legal", form);
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/capital/legal/${form.id}`, form);
      await fetching();
      makeToast("success", res?.data?.message);
    } catch (error) {
      throw error;
  ...
  const confirmDelete = async (item: Capital) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/capital/legal/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
