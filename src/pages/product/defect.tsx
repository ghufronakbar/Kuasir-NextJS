...
const useDefects = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await api.get<Api<DetailDefect[]>>("/product/defect");
  ...
  const create = async () => {
    try {
      const res = await api.post("/product/defect", {
        ...form,
      });
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/product/defect/${form.id}`, {
        ...form,
      });
      await fetching();
      makeToast("success", res?.data?.message);
  ...
  const confirmDelete = async (item: DetailDefect) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/product/defect/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
