...
const useOutcomes = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await api.get<Api<DetailOutcome[]>>("/product/outcome");
  ...
  const create = async () => {
    try {
      const res = await api.post("/product/outcome", {
        ...form,
  ...
  const confirmDelete = async (item: DetailOutcome) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/product/outcome/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
