              ...
              <tr className="bg-white border-b  border-gray-200" key={index}>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-row gap-2 font-medium">
                    <Link href={`/product/menu/${item.id}`}>
...
export default AuthPage(MenuPage, ["CASHIER", "OWNER"]);
...
const useProducts = () => {
  ...
  const fetchParents = async () => {
    const res = await api.get<Api<DetailProduct[]>>(
  ...
  const create = async () => {
    try {
      const res = await api.post("/product/menu", {
        ...form,
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/product/menu/${form.id}`, {
        ...form,
        productCategory: isOther ? other : form.productCategory,
      });
      await fetching();
  ...
  const confirmDelete = async (item: DetailProduct) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/product/menu/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
