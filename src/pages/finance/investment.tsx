...
const THEAD = ["No", "Investment", "Note", "Amount", "Created At", ""];
            ...
            <Dialog.Positioner>
              <Dialog.Content>
                ...
                <Dialog.Body>
                  ...
                  >
                    <Label className="mt-2 font-medium">Investment</Label>
                    <input
...
export default AuthPage(InvestmentPage, ["CASHIER", "OWNER"]);
...
const useInvestment = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const response = await api.get<Api<Transaction[]>>("/finance/investment");
  ...
  const create = async () => {
    try {
      const res = await api.post("/finance/investment", form);
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/finance/investment/${form.id}`, form);
      await fetching();
      makeToast("success", res?.data?.message);
    } catch (error) {
      throw error;
  ...
  const confirmDelete = async (item: Transaction) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/finance/investment/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
