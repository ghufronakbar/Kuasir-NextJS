              ...
              <Dialog.Content>
                <Dialog.Header>
                  <Dialog.Title className="font-semibold">
                    {form.id === "-" ? "Create New Dividend" : "Edit Dividend"}
                  </Dialog.Title>
...
export default AuthPage(DividendPage, ["CASHIER", "OWNER"]);
...
const useDividend = () => {
  ...
  const create = async () => {
    try {
      const res = await api.post("/finance/dividend", form);
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/finance/dividend/${form.id}`, form);
      await fetching();
      makeToast("success", res?.data?.message);
    } catch (error) {
      throw error;
  ...
  const confirmDelete = async (item: Finance) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/finance/dividend/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
