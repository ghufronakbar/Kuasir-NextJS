              ...
              <Dialog.Content>
                <Dialog.Header>
                  <Dialog.Title className="font-semibold">
                    {form.id === "-" ? "Create New Loan" : "Edit Loan"}
                  </Dialog.Title>
                ...
                <Dialog.Body>
                  ...
                  >
                    <Label className="mt-2 font-medium">Loan</Label>
                    <select
...
export default AuthPage(LoanPage, ["CASHIER", "OWNER"]);
...
const useLoan = () => {
  ...
  const create = async () => {
    try {
      const res = await api.post("/finance/loan", form);
      await fetching();
  ...
  const edit = async () => {
    try {
      const res = await api.put(`/finance/loan/${form.id}`, form);
      await fetching();
      makeToast("success", res?.data?.message);
    } catch (error) {
      throw error;
  ...
  const confirmDelete = async (item: Finance) => {
    try {
      ...
      setLoading(true);
      const res = await api.delete(`/finance/loan/${item.id}`);
      await fetchData();
      makeToast("success", res?.data?.message);
      setLoading(false);
    } catch (error) {
